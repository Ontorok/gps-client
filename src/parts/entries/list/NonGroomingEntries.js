...
const NonGroomingEntries = ({ sortedColumn, sortedBy, onSort }) => {
  ...
  useEffect(() => {
    const fetchNonGroomingEntries = async () => {
      try {
        const res = await axiosInstance.get(ENTRIES_API.fetch_all_non_grooming_entries, { params: { page, perPage } });
