...
Mock.onGet(USERS_API.fetch_all).reply(config => {
  const {
    params: { page, perPage }
  } = config;
  const filtered = userDB.users.slice((page - 1) * perPage, (page - 1 + 1) * perPage);
  return [200, { succeed: true, totalRows: userDB.users.length, result: filtered }];
});
Mock.onGet(USERS_API.fetch_all_archive).reply(config => {
  const {
    params: { page, perPage }
  } = config;
  const archiveUsers = userDB.users.filter(user => !user.isActive);
  const filtered = archiveUsers.slice((page - 1) * perPage, (page - 1 + 1) * perPage);
  return [200, { succeed: true, totalRows: archiveUsers.length, result: filtered }];
});
